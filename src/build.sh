...
for branch in ${BRANCH_NAME//,/ }; do
  ...
  if [ -n "$branch" ] && [ -n "$devices" ]; then
    ...
    # No more need for signature spoofing patch, but disable the debugging check
    cd frameworks/base
    patch --quiet --force -p1 -i "/root/signature_spoofing_patches/android_frameworks_base-Android13.patch"
    git clean -q -f
    cd ../..
